...
function buildInjectedScript(quiz) {
  ...
  <script>
    ...
    // Tells the quiz engine to persist pause/resume progress to the account
    // (via /api/progress) instead of only to this browser's localStorage.
    quizData.quizKey = ${JSON.stringify(quiz.key)};
    quizData.serverProgress = true;
    ...
    (function () {
      ...
      finishTestInternal = function (dest) {
        ...
        try {
          questions.forEach(function (q, i) {
            ...
            answers.push({
              ...
              peeked: card.dataset.peeked === "true",
            });
          });
        } catch (e) { /* best-effort */ }
        ...
        var durationSeconds = typeof elapsedSeconds === "number" && elapsedSeconds > 0
          ? elapsedSeconds
          : Math.max(0, Math.round((Date.now() - __startedAt) / 1000));
