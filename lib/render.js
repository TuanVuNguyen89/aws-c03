...
function resumeChips(entries) {
  if (!entries || entries.length === 0) return "";
  return `<div class="resume-chips">` + entries.map((p) => `
    <a class="resume-chip" href="/quiz/${escapeHtml(p.quizKey)}" title="Tiếp tục ${escapeHtml(modeLabel(p.mode))} — lưu lúc ${escapeHtml(p.updated_at)}">
      <i class="fas fa-rotate-left"></i> Tiếp tục ${escapeHtml(modeLabel(p.mode))}
      <span class="resume-chip-count">${p.answered}/${p.total || "?"}</span>
    </a>`).join("") + `</div>`;
}

function quizCard(q, best, progress) {
  const bestPercent = best ? best.best_percent : null;
  const attempts = best ? best.attempts : 0;
  const entries = (progress || []).map((p) => ({ ...p, quizKey: q.key }));
  return `
  <div class="quiz-card">
    ...
    ${resumeChips(entries)}
    <a class="btn" href="/quiz/${q.key}">${entries.length ? "Mở bài test" : "Start test"}</a>
  </div>`;
}

function dashboardPage({ user, quizzes, bestScores, inProgress }) {
  const prog = inProgress || {};
  const cards = quizzes.map((q) => quizCard(q, bestScores[q.key], prog[q.key])).join("\n");
  return layout({
...
function attemptDetailPage({ user, attempt, quiz }) {
  ...
  const items = answers.map((a) => {
    ...
    const marks = [
      a.flagged ? `<span class="q-mark flagged"><i class="fas fa-flag"></i> Đã đánh dấu</span>` : "",
      a.peeked ? `<span class="q-mark peeked"><i class="fas fa-eye"></i> Đã xem đáp án</span>` : "",
    ].join("");
    ...
    <div class="review-item">
      ...
      ${marks ? `<div class="q-marks">${marks}</div>` : ""}
      ${optionsHtml}
